...
void lanczos_algo(sycl::buffer<int> a_row_buf, sycl::buffer<int> a_columns_buf,
                  ...
                  double *orth_vec, sycl::buffer<double> orth_vec_pre_buf,
                  int m, int size, sycl::queue queue) {
  size_t global_size = ((size + BLOCKSIZE - 1) / BLOCKSIZE) * BLOCKSIZE;

  sycl::nd_range nd_range =
      sycl::nd_range(sycl::range(global_size), sycl::range(BLOCKSIZE));
  ...
  for (int i = 0; i < m; i++) {
    ...
    if (fabs(beta[i] - 0) > EPS) {
      sycl_mtx_sclr_mul(w_buf, 1 / beta[i], orth_vec_buf, size, queue,
                        nd_range);
    } else {
      ...
      sycl_mtx_sclr_mul(orth_vec_buf, 1 / norm_val, orth_vec_buf, size, queue,
                        nd_range);
    }
    ...
    if (i == 0) {
      ...
      sycl_calc_w(w_buf, alpha[i], orth_vec_buf, orth_vec_pre_buf, beta[i],
                  size, queue, nd_range);
    }
  }
}
